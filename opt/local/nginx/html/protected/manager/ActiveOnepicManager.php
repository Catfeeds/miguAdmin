<?php 
class ActiveOnepicManager extends WxGuide
{
	...
	public static function getClicks($keyword,$first,$end,$province,$city){
		$res = array();
		$sql_select = "select province,city,cp,epg,title,max(from_unixtime(cTime)) as time,min(from_unixtime(cTime)) as mintime,count(*) as total";
		$sql_from = " from yd_active_onepic ";
		if(!empty($keyword) && !empty($province) && !empty($city)){
			$sql_where = " where title like '%".$keyword."%' and (cTime >= $first and cTime <= $end  ) and province='$province' and city='.$city.'";
		}else if(!empty($keyword) && !empty($province)){

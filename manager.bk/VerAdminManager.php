 ...
 * Time: 20:24
 */
class VerAdminManager extends VerAdmin{

    public static function getNickName($str){
        if(empty($str)) return '未知错误';
        if(is_numeric($str)){
            $admin = VerAdmin::model()->findByPk($str);
        }else{
    ...
    public static function getAdmin($name){
        ...
        return VerAdmin::model()->find($criteria);
    }
    ...
    public static function getAdminList($data){
        ...
        $sql_from = ' from yd_ver_admin as a inner join yd_Ver_group as g on g.id=a.auth';

...
<div class="modules">
    ...
    <span><input type="submit" name="button" id="button" onclick='search()' class="seo-gray" value="搜索"></span>
...
function search(){	
    ...
    ({
        ...
        type:"get",
        url:"/admin/setting/checkData?mid=<?php echo $this->mid;?>&nid=<?php echo $_GET['nid'];?>&provinceCode="+shengid[0]+"&cityCode="+cityid[0],
        success:function(data)
        {
            data = eval(data);
            var num = parseInt(data[0].flag);
//            console.log(num);
            if( num > 0 ){
                window.location.href = '/admin/setting/index.html?mid=<?php echo $this->mid?>&nid=<?php echo $_GET['nid']?>&type='+val+'&provinceCode='+shengid[0]+'&cityCode='+cityid[0];
            }else if(num <= 0){
                var r = confirm("您搜索的省市没有数据，按确定进入后会自动生成数据，按取消中止操作");
                if(r == true ){
                    window.location.href = '/admin/setting/index.html?mid=<?php echo $this->mid?>&nid=<?php echo $_GET['nid']?>&type='+val+'&provinceCode='+shengid[0]+'&cityCode='+cityid[0];
		    //return false;
                }else{
...
$('.modules a').click(function(){
    ...
    if(img.length !== 0){
        ...
        $.getJSON('<?php echo $this->get_url('setting','photo')?>',{val:v,type:type,provinceCode:pcode,cityCode:ccode},function(d){
            if(d.code == 200){
                layer.close(my);
                layer.open({
                    type: 1,
        ...
        if(empty(v)) return false;
        var my = layer.msg('加载中', {icon: 16,shade:0.3});
        $.getJSON('<?php echo $this->get_url('setting','upload')?>',{val:v,type:type,fid:fid,provinceCode:pcode,cityCode:ccode},function(d){
            if(d.code == 200){
                layer.close(my);
                layer.open({
                    type: 1,
    ...
    function getregin(){
        ...
        $.getJSON("/admin/setting/getcity?mid=<?php echo $_GET['mid']?>",{id:i[0]},function(data){

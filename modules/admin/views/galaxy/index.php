...
//    alert(sheng);
        ...
        window.location.href = '/admin/galaxy/index.html?mid=<?php echo $this->mid?>&nid=<?php echo $_GET['nid']?>&type='+val+'&provinceCode='+shengid[0]+'&cityCode='+cityid[0];
    });
    ...
    $('.modules a').click(function(){
        ...
        if(img.length !== 0){
            ...
            $.getJSON('<?php echo $this->get_url('galaxy','photo')?>',{val:v,type:type,provinceCode:pcode,cityCode:ccode},function(d){
                if(d.code == 200){
                    layer.close(my);
                    layer.open({
                        type: 1,
            ...
            if(empty(v)) return false;
            var my = layer.msg('加载中', {icon: 16,shade:0.3});
            $.getJSON('<?php echo $this->get_url('galaxy','upload')?>',{val:v,type:type,fid:fid,provinceCode:pcode,cityCode:ccode},function(d){
                if(d.code == 200){
                    layer.close(my);
                    layer.open({
                        type: 1,

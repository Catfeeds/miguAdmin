...
//	echo $url;die();
	$ch = curl_init();
        ...
        $output = curl_exec($ch);
//	if ($output  === false) {
  //        return  "cURL 具体出错信息: " . curl_error($ch);
    //    }
//if(curl_errno($ch))
//		{
...
//        $result = $command->execute();
	...
	$json=array('url'=>$tpurl);
        echo json_encode($json);	


        $this->redirect($tpurl);
    }
	public function actionGetWx(){
        if (empty($_REQUEST['stbid']))           throw new ExceptionEx('序列号不能为空');
        $access_token = $this->getToken();
...
//	$this->redirect($tpurl);
    ...
    public function handleEvent($object)
    {
        ...
        switch ($object->Event)
        {
		    ...
		    $as = Wxbox::model()->find("number = '$openid'");
                    $arr = $this->getIds($con,$output);
                    if($arr){
                        if(isset($as)){
                            $contentStr = "设备绑定成功!";
                        }else{
                            $contentStr = "设备绑定成功!".$contentStr;
                        }
                    }else{
                        $contentStr = "设备绑定失败!";
                    }
...
//                break;
            case "CLICK":
                ...
                if($click['description']=='客服'){
                    ...
                    $ni = date('Y-m-d 09:00:00' ,strtotime('now'));
                    $er = date('Y-m-d 22:00:00' ,strtotime('now'));
                    if($now > $ni && $now <$er){
                        $result = $this->transmitService($object);
                        return $result;
                    }else{
                        $result = $this->huiFu($object);
                        return $result;
                    }
                }
...
//            $message = $command->queryAll();
    ...
    public function huiFu($object){
        $xmlTpl = "<xml>
                    <ToUserName><![CDATA[%s]]></ToUserName>
                    <FromUserName><![CDATA[%s]]></FromUserName>
                    <CreateTime>%s</CreateTime>
                    <MsgType><![CDATA[text]]></MsgType>
                    <Content><![CDATA[魔百和客服人员工作时间为9:00-22:00点，麻烦您在工作时间内进行咨询，给您带来不便还望谅解]]></Content>
                    </xml>";

<?php
    class KefuController extends WController{
        ...
        public function actionHistory(){
            ...
            $data = WxboxManager::getAll();
            foreach($res['list'] as $key=>$val){
                $res['list'][$key]['province']='';
                $res['list'][$key]['city']='';
                foreach($data as $k=>$v){
                    if($val['openid']==$v['number']){
                        $res['list'][$key]['openid']=$v['name'];
                        $res['list'][$key]['province']=$v['provinceName'];
                        $res['list'][$key]['city']=$v['cityName'];
                    }
                }
            }
            /*echo '<pre>';
        ...
        public function actionOut(){
            ...
            $first=1462896000;
            $end = 1462982399;*/
            $res = WxHistoryManager::getExcel($worker,$first,$end);
            $data = WxboxManager::getAll();
            foreach($res as $key=>$val){
                ...
                foreach($data as $k=>$v){
                    if($val['openid']==$v['number']){
                        $res[$key]['openid']=$v['name'];
                        $res[$key]['province']=$v['provinceName'];
                        $res[$key]['city']=$v['cityName'];
                    }
                }
            }
            foreach($res as $key=>$val){
                ...
                $arr[$key]['ctime']=$val['ctime'];
                $arr[$key]['openid']=$val['openid'];
                $arr[$key]['province']=$val['province'];
                $arr[$key]['city']=$val['city'];
                $arr[$key]['text']=$val['text'];
                $arr[$key]['worker']=$val['worker'];
            }
        ...
        public function Excel($arr){
            ...
            $tableheader = array('编号','日期','录单时间','微信昵称','省份','地市','用户描述','客服工号');
            //填充表头信息

...
<!--                <span>当类型为“下发消息（除文本消息）”或“跳转图文消息URL”时有效，并为必填项</span>-->
	    ...
	    <div class="class_con">
                <label for="">state值</label>
                <input type="text" name="state" id="state" value="<?php echo !empty($wxmenu->state)?$wxmenu->state:''?>" maxlength="255"  onblur="onblur_url()" />
                <span>当链接分牌照方时，并为必填项</span>
                <span style="color:#F00;" id="span_url"></span>
            </div>
        </div>

...
<?php $template = !empty($arr['templateType']) ? $arr['templateType'] : '';
		if ($template != 'E') {
			echo " .dir{color:red}";
		}
    ?>
...
<div style="height:20px;width:300px;float: left">全量数据>内容库管理>查看</div>
...
<script type="text/javascript">
        ...
        var p =1;
        $.getJSON('<?php echo $this->get_url('content','keylist')?>
			',{p:p},function(d){
			if(d.code == 200){
			layer.open({
			type: 1,
			...
			var vid = "
<?php echo $_REQUEST['vid']?>
	";
	if(confirm("批量设置默认播放会刷新页面！！")){
	$.post("
<?php echo $this->get_url('content','allplay')?>
	",{vid:vid},function(d){
	location.reload();
	})
	}
	...
	$.post("
<?php echo $this->get_url('content','changetitle')?>
	",{id:id,title:title},function(d){
	location.reload();
	})
	})
...
<?php echo $_REQUEST['vid']?>
	";
	$.post("
<?php echo $this->get_url('content','allsubmit') ?>
	",{vid:vid},function(d){
	location.reload();
	})
	})
	...
	$.getJSON("
<?php echo $this->get_url('content','alert')?>
	",{id:id},function(d){
	if(d.code==200){
	layer.open({
	type: 1,
...
<?php echo $_REQUEST['vid']?>
	...
	$.getJSON('
<?php echo $this->get_url('content','upload')?>
	',{vid:vid,cp:cp,title:title},function(d){
	if(d.code == 200){
	layer.close(my);
	layer.open({
	type: 1,
	...
	$.post("
<?php echo $this->get_url('content','submit')?>
	",{id:id,flag:flag,info:info,ShowType:ShowType},function(d){
	location.reload();
	})
	})
	$('.del').click(function(){
	var id = $(this).parent().parent().children('input').val();
	$.post("
<?php echo $this->get_url('content','del')?>
	",{id:id},function(d){
	location.reload();
	})
	})
	...
	var id = $(this).parent().parent().children('input').val();
	$.post("
<?php echo $this->get_url('content','order')?>
	",{id:id,order:order},function(d){
	//location.reload();
	})
	})
	/*function repic(obj){
...
<?php echo $_REQUEST['vid']?>
	";
	$.post("
<?php echo $this->get_url('content','pictype') ?>
	",{vid:vid,flag:flag},function(d){
	location.reload();
	})
	}*/
	...
	var id = $(this).parent().parent().children('input').val();
	$.post("
<?php echo $this->get_url('content','delete')?>
	",{id:id},function(d){
	location.reload();
	})
	})
...
<?php echo $_REQUEST['vid']?>
	";
	$.getJSON('
<?php echo $this->get_url('content','coladd')?>
	',{vid:vid},function(d){
	if(d.code == 200){
	layer.open({
	type: 1,
	...
	var id = $(this).siblings('input').val();
	$.getJSON("
<?php echo $this->get_url('content','pic')?>
	",{id:id},function(d){
	if(d.code==200){
	layer.open({
	type: 1,
	...
	//window.location.href="/version/content/index?mid="+"
<?php echo $this->mid?>
	";
	})
</script>

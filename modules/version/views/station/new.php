<style type="text/css">
    ...
    .m-318-171{width:250px;  height:115px;  margin-bottom: 20px;  border:1px solid #ccc;  border-radius: 8px;  float:left;}
    .m-825-429{width:580px;  height:250px;  border:1px solid #ccc;  border-radius: 8px;  float:left;}
    .m-492-357{width:280px;  height:182.5px;  border:1px solid #ccc;  border-radius: 8px;  float:left;  margin-bottom: 20px; }
    #pageType{display:none;}
    .page2{display:none;}
    .typeUrl{display:none;}
    #url{display:none;}
	/*#main div {position: relative;}*/
	.uploadfy{position:absolute}
    <!--    --><?php
...
<!--                <span class="infoSpan">请上传宽为--><?php //echo $_GET['width']*250+($_GET['width']-1)*20 ;?><!--，高为--><?php //echo $_GET['height']*105+($_GET['height']-1)*20;?><!--的图片！</span>-->
    ...
    <tr>
        <td align="center" colspan="2">
            ...
            <input type="button" value="取消" class="gray" onclick="window.close();">
...
//        console.log($(img));
//        return false;
        window.location.href = "/version/screen/Banner/mid/"+"<?php echo $this->mid?>"+"/nid/"+gid+"/epg/"+epg+"/order/"+order+'/width/'+width+'/height/'+height+'/x/'+x+'/y/'+y+'/nid/'+nid;
    });
    $(".uploadify").position("absolute");
</script>

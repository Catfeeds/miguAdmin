...
$stationid = !empty($_GET['topid'])?$_GET['topid']:"7";
$sql = "SELECT
	t1.type,
	t1.workid,
	t1.uid
...
<script type="text/javascript" src="/js/jquery.bxslider.min.js"></script>
    ...
    function getauth(sitelist_id)
    {
        ...
        ({
            type:'get',
            async:false,
            url:'/version/screen/mid/'+mid+'/sitelist_id/'+sitelist_id,
        ...
        if(<?php echo $estatus ?> == 1 && <?php echo $show ?> == 1){
          ...
          if(x == 500){
            ...
            if($(this).html() !== '删除') {
                ...
                $.getJSON('<?php echo $this->get_url('site','upload')?>', {val: v,id:id, gid: gid,show:<?php echo $show ?>}, function (d) {
                    if (d.code == 200) {
                        layer.close(my);
                        layer.open({
                            type: 1,
...
function add(obj)
        {
            ...
            if($(obj).parent().is('div') && <?php echo $estatus ?> == 0 ){
                ...
                var id = $(obj).attr('id');
		window.open('/version/station/updateContent/show/<?php echo $show ?>/mid/'+mid+'/width/'+width+'/height/'+height+'/x/'+x+'/y/'+y+'/order/'+order+'/id/'+id+'/nid/'+nid);

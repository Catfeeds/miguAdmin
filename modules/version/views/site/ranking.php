...
<table class="mtable" width="100%" cellspacing="0" cellpadding="10" style='overflow-y:scroll;'>
    ...
    <tr class="utp" style="display:none">
        ...
        <td><select name="uType" class="form-input w300" id="uType">
                ...
                <option  value="B">多集数字详情页</option>
                <option  value="C">多集标题详情页</option>
                <option  value="D">横图单片详情页</option>
    ...
    <?php
        if(empty($imgFlag)){?>
            ...
            <tr>
                ...
                <td>
                    <div class="up-main" id="main-ranking">
                        <?php if(!empty($appFlag)){?>
    ...
    $('#upload_file').uploadify({
        ...
        'onUploadSuccess' : function(file, data, response) {//每次成功上传后执行的回调函数，从服务端返回数据到前端
            ...
            if(value.code == 200){
                ...
                var l = $('#main-ranking').find('#upImg').find('img');
                if(l.length < 1){
                    $('#main-ranking').find('#upImg').append('<img src="'+value.url+'" width="100%" height="100%" id="tupian">');
                }else{
...
//        G.id = $('input[name=id]').val();
        ...
        $.post('<?php echo $this->get_url('site','RankingAdd')?>',G,function(d){
            if(d.code == 200){
                location.reload();
            }else{
                layer.close(load);

        ...
        $html = str_replace("131px", "95px", $html);
	        ...
	        $b = "/点击修改<\/a><a[\s\S]*?删除<\/a>/";
                ...
                foreach ($matches[0] as $key => $value) {
                        $html = str_replace($matches[0][$key], "查看</a>", $html);
                }
                ...
                <div>
                    ...
                    if(!empty($topList)){
                        ...
                        foreach ($topList['list'] as $k=>$v) {
                            ...
                            <div class="centerTop">
                                <div class="topPic">
                                    <span class="editTop" onclick="editTop(this)" uiId="<?php /*var_dump($v);die;*/echo $v[0]['id']; ?>" position="<?php echo $v[0]['position']; ?>">查看</span>
                                    ...
                                    <?php foreach ($v as $key=>$val) {
                                        if($key>0){
                                            ...
                                            <li class="lit" style="background:white;">
                                                <span class="editTop" onclick="editTop(this)" uiId="<?php echo $val['id'] ?>" order='<?php echo $val['scort'];?>' position="<?php echo $v[0]['position']; ?>">查看</span>
                    ...
                    <div class="centerTopApp">
                        ...
                        <ul class='topUl'>
                            <?php foreach ($list['res'] as $k=>$v){?>
                                <li class="lit" style="background:white;">
                                    <span class="editTop" appFlag="1" onclick="editTop(this)" uiId="<?php echo $v['id'] ?>" order='<?php echo $v['scort'];?>'>查看</span>
                                    ...
                                    <ul class="topUl">
                                        <?php foreach($news as $k=>$v){?>
                                            <li class="lit" style="background: #0b93d5;">
                                                <span class="editTop" onclick="editTop(this)" uiId="<?php echo $v['id'] ?>">查看</span>
...
//            console.log(maxAddYiJi);
	    ...
	    var auth = "<?php echo $_SESSION['auth']?>";
            ...
            if($(this).html() !== '删除') {
                ...
                $.getJSON('<?php echo $this->get_url('site','uploadshow')?>', {val: v,id:id, gid: gid}, function (d) {
                    if (d.code == 200) {
                        layer.close(my);
                        layer.open({
                            type: 1,
        ...
        if(imgFlag){
                //alert('2');
                $.getJSON('<?php echo $this->get_url('site','rankingShowView')?>', {id:id,mid:mid}, function (d)
                {
                    if (d.code == 200) {
                        layer.open({
                            type: 1,
                ...
                $.getJSON('<?php echo $this->get_url('site','rankingShowView')?>', {id:id,mid:mid,imgFlag:1,appFlag:1}, function (d)
                {
                    if (d.code == 200) {
                        layer.open({
                            type: 1,
                ...
                $.getJSON('<?php echo $this->get_url('site','rankingShowView')?>', {id:id,mid:mid,imgFlag:1}, function (d)
                {
                    if (d.code == 200) {
                        layer.open({
                            type: 1,
...
function add(obj)
		...
		return false;
                ...
                var id = $(obj).attr('id');
		window.open('/version/station/updateContentshow/mid/'+mid+'/width/'+width+'/height/'+height+'/x/'+x+'/y/'+y+'/order/'+order+'/id/'+id+'/nid/'+nid);

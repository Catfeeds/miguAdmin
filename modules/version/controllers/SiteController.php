...
//		echo '<pre>';
	        if(!empty($list)){
		   $this->render('topic',array('bkimg'=>$bkimg,'html'=>$html,'topList'=>$list,"result"=>$res));
	        }else{
	...
	//	var_dump($list);die;
                if(!empty($list)){
                   $this->render('topic',array('bkimg'=>$bkimg,'html'=>$html,'news'=>$list,"result"=>$res));
                }else{
                   $this->render('topic',array('bkimg'=>$bkimg,'html'=>$html,"result"=>$res));
                }
	    }elseif($type=='4'){
...
//		var_dump($list);die;
	        if(!empty($list)){
		   $this->render('topic1',array('bkimg'=>$bkimg,'html'=>$html,'topList'=>$list,"result"=>$res));
	        }else{
	...
	//	var_dump($list);die;
                if(!empty($list)){
                   $this->render('topic1',array('bkimg'=>$bkimg,'html'=>$html,'news'=>$list,"result"=>$res));
                }else{
                   $this->render('topic1',array('bkimg'=>$bkimg,'html'=>$html,"result"=>$res));
                }
	    }elseif($type=='4'){
...
//		var_dump($list);die;
	        if(!empty($list)){
		   $this->render('topic2',array('bkimg'=>$bkimg,'html'=>$html,'topList'=>$list,"result"=>$res));
	        }else{
	...
	//	var_dump($list);die;
                if(!empty($list)){
                   $this->render('topic2',array('bkimg'=>$bkimg,'html'=>$html,'news'=>$list,"result"=>$res));
                }else{
                   $this->render('topic2',array('bkimg'=>$bkimg,'html'=>$html,"result"=>$res));
                }
	    }elseif($type=='4'){
    ...
    public function actionUpload(){
        ...
        if(empty($_REQUEST['id'])){
            $mvui = VerUiCopyManager::getAll($_GET['gid']);
        }else{
            $mvui = VerUiCopyManager::getData($_GET['gid'],$pos);
        }
    ...
    public function actionAdd(){
        ...
        $mvui = VerUiCopy::model()->findByAttributes(array('position'=>$_POST['position'],'id'=>$_POST['id']));
    ...
    public function actionDel(){
        $id = $_REQUEST['id'];
        $result = VerUiCopy::model()->findByPk($id);
		$result->type = 3;
		$result->flag = 7;
        if($result->save()){
	...
	$time = time();
		...
		$res = VerSite::model()->updateAll(array('status'=>$status,'upTime'=>$time),'id=:id',array(':id'=>$id));
            ...
            foreach($result as $k=>$v){
               ...
               $res1 = VerSite::model()->updateAll(array('status'=>$status,'upTime'=>$time),'vid=:vid',array(':vid'=>$vid));
            }
            $res = VerSite::model()->updateAll(array('status'=>$status,'upTime'=>$time),'id=:id',array(':id'=>$id));
        }
        }
	...
	if(!empty($res) || !empty($res1) || !empty($result)){
	    echo '200';	
	}else{
	    echo '500';	
	}	
...
//        var_dump($tmp);die;
   ...
   public function topIndex($gid)
    ...
    {
	$sql4 = "select pic,title,id ,position from yd_ver_ui_copy where gid=$gid and `delFlag`=0  ORDER BY `scort` asc";
	 ...
	 public function topIndexNewstrue($gid)
    {
    ...
    {
        $gid = $_REQUEST['nid'];
        $sql = "select position from yd_ver_ui_copy where gid = $gid AND `delFlag`=0 order by `position` DESC limit 1 ";
        $res = SQLManager::queryAll($sql);
        $sql2 = "select count(*) from yd_ver_ui_copy where gid=$gid AND `delFlag`=0";
    ...
    {
        $id = $_REQUEST['id'];
	$show = !empty($_REQUEST['show'])?$_REQUEST['show']:"";
        $sql = "select * from yd_ver_ui_copy where id = $id AND delFlag=0 ";
        $res = SQLManager::queryAll($sql);
        if(empty($_REQUEST['imgFlag'])){
            $n = $this->renderPartial(
                'updateRanking',
     ...
     public function actionRankingShowView()
    {
        $id = $_REQUEST['id'];
...
//        $mvui->pic      = 'http://localhost:8081/file/' . trim(substr($_POST['key'],-36));
   ...
   //     var_dump($_POST);die;
        $id = $_POST['uiId'];
        $mvui = VerUiCopy::model()->findByPk($id);
...
//        $mvui->scort    = trim($_REQUEST['order']);
	if(strlen($_POST['key'])>1){
        ...
        $pic = FTP_PATH . trim(substr($_POST['key'],-36));	
        $mvui->pic      = $pic;
        //$mvui->pic = FTP_PATH . trim(substr($_POST['key'],-36));
	//var_dump(trim(substr($_POST['key'],-36)));die;
        //$mvui->pic      = 'http://localhost:8081/file/' . trim(substr($_POST['key'],-36));
        if(!$mvui->save()){
    ...
    public function actionPopmsgUpdate(){
        $id=$_REQUEST['uiId'];//id
        $info=SpecialTopic::model()->findByPk($id);
        $info->title=$_REQUEST['title'];
        $info->action=$_REQUEST['action'];
        $info->param=$_REQUEST['param'];
        $info->type=$_REQUEST['type'];
        $info->uType=$_REQUEST['uType'];
        $info->sid=$_REQUEST['sid'];
        $info->tType=$_REQUEST['tType'];
        $info->picSrc=$_REQUEST['key'];
        $info->sid=$_REQUEST['sid'];
        if($info->save()){
    ...
    public function actionEditBanner(){
        //$id=$_REQUEST['id'];
        //$data=SpecialTopic::model()->findByPk($id);
        $this->render("editbanner");
    }
    public function actionDelBanner(){
        $id=$_POST['id'];
        $result = SpecialTopic::model()->deleteByPk($id);
        if($result){
            echo json_encode(array('code'=>200,'msg'=>'删除成功'));
        }else{
            echo json_encode(array('code'=>404,'msg'=>'删除失败'));

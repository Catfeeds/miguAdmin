...
class MessageController extends Controller{
    ...
    public function actionIndex(){
        ...
        $this->render('index');*/
//        var_dump(base64_decode(urldecode($_REQUEST['image'])));
//	echo "<hr />";
//	var_dump($_REQUEST['openid']);
//die;
        if(empty(Yii::app()->session['user'])){
            $user_info['openid'] = $_REQUEST['openid'];
            $user_info['image']=base64_decode(urldecode($_REQUEST['image']));
    ...
    public function actionAdd(){
        ...
        /*$sql = "select * from yd_wxbox where number = '{$_POST['number']}'";
        $list = Yii::app()->db->createCommand($sql)->queryAll();
        $this->render('index',array('list'=>$list));*/

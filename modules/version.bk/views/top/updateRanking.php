...
<table class="mtable" width="100%" cellspacing="0" cellpadding="10">
    ...
    <tr>
        ...
        <td>
            <select name="tType" class="form-input w300" id="tType" onchange="aa()">
                ...
                <option <?php $tType = !empty($res['tType'])?$res['tType']:'';if($tType=='2'){echo "selected=selected"; }?> value="2" >应用</option>
                <option <?php $tType = !empty($res['tType'])?$res['tType']:'';if($tType=='3'){echo "selected=selected"; }?> value="3" >牌照方</option>
                <option <?php $tType = !empty($res['tType'])?$res['tType']:'';if($tType=='4'){echo "selected=selected"; }?> value="4" >第三方</option>
            </select>
        </td>
    </tr>
    ...
    <tr class="utp" style="display:none">
        ...
        <td><select name="uType" class="form-input w300" id="uType">
                ...
                <option <?php $uType = !empty($res['uType'])?$res['uType']:'';if($uType=='2'){echo "selected=selected"; }?> value="2">栏目</option>
                <!--<option  value="3">详情</option>-->
                <option <?php $uType = !empty($res['uType'])?$res['uType']:'';if($uType=='4'){echo "selected=selected"; }?> value="4">专题</option>
                ...
                <option <?php $uType = !empty($res['uType'])?$res['uType']:'';if($uType=='A'){echo "selected=selected"; }?> value="A">电影详情页</option>
                <option <?php $uType = !empty($res['uType'])?$res['uType']:'';if($uType=='B'){echo "selected=selected"; }?> value="B">电视剧详情页</option>
                <option <?php $uType = !empty($res['uType'])?$res['uType']:'';if($uType=='D'){echo "selected=selected"; }?> value="D">单片横图模板</option>
                <option <?php $uType = !empty($res['uType'])?$res['uType']:'';if($uType=='C'){echo "selected=selected"; }?> value="C">综艺详情页</option>
...
//        console.log(G);return false;
        ...
        $.post('<?php echo $this->get_url('top','RankingUpdate')?>',G,function(d){
            if(d.code == 200){
                location.reload();
            }else{
                layer.close(load);

            ...
            if(!empty($list)){
                foreach($list as $l){?>
                    ...
                    ?>'>
                        ...
                        <td><?php
                            if(in_array('2',$res['status']) || $_SESSION['auth']=='1'){
                                echo "<a class='status_true'>上线&nbsp;</a><a class='status_false'>下线&nbsp;</a>";
                            }else{
                                echo "<a>上线&nbsp;</a><a>下线&nbsp;</a>";
                            }
                            ?>
...
//            console.log(maxAddYiJi);
    ...
    if(leftNavFlag=='0'){
        ...
        for(var i = 0 ; i<max ; i++){
            ...
            for(var j = 0 ; j< maxTwo ; j++){
                if(j == twoId){
                    $_this.attr('src','/file/button/folder_true.png');
                    $_this.parent().next('ul').slideDown(100).children('li');
                }else{
                    $_this.attr('src','/file/button/folder_false.png');
...
//            window.location.href=test+"&status="+1+"&page=1"
    ...
    $('.search_title').click(function(){
        ...
        var nid = "<?php echo $_GET['nid']?>";
        var gid = "<?php echo $_REQUEST['nid']?>";
        var headerUrl = "/version/site/index/mid/<?php echo $this->mid;?>"+"/gid/"+gid+'/nid/'+nid;
        var centerUrl = '';
        if(!empty(title)){

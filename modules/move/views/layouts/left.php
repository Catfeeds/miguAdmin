                ...
                foreach($nav as $v){
                    ...
                    if(!empty($mid) && $mid == $v->pid){
                        ...
                        <li class="<?php echo !empty($_GET['nid']) && $_GET['nid'] == $v['id']?'thismenu':''?>">
                            <span><a href="<?php echo $v['url'] == '#'?'#':Yii::app()->createUrl($v['url'],array('mid'=>$_GET['mid'],'nid'=>$v['id'],'epg'=>$v['name'],'pro'=>$admin['nickname']))?>" class="<?php echo !empty($_GET['nid']) && $_GET['nid'] == $v['id']?'hovered':''?>"><?php echo $v['name']?></a></span>
                        </li>
                    <?php
                    }
                }

...
<table class="mtable" width="100%" cellspacing="0" cellpadding="10">
    <tr width="900px">
        <?php
        if(count($ui) == 1){
            echo "<td fid='0' id=".$ui[0]['position']." width='250px'><img src=".$ui[0]['pic']." width='80%'><a pos=".$ui[0]['position']." href=".'javascript:void(0)'.">点击修改</a><a href=".'javascript:void(0)'." dss=".$ui[0]['id']." style='margin-left:250px' class='dss' pos=".$ui[0]['position'].">删除</a></td>";
            ...
            foreach ($ui as $key => $val) {
                echo "<td fid='$key' id=".$val['position']." width='150px'><img src=".$val['pic']." width='80%'><a pos=".$val['position']." href=".'javascript:void(0)'.">点击修改</a><a href=".'javascript:void(0)'." pos=".$val['position']." class='dss' dss=".$val['id']." style='margin-left:250px'>删除</a></td>";
            }

            echo "<td id='shang' width='200px' fid='2'><a pos=".$ui[0]['position']." href=".'javascript:void(0)'.">点击上传</a></td>";
        }elseif(count($ui)==3){
            foreach ($ui as $key => $val) {
                echo "<td fid='$key' id=".$val['position']." width='150px'><img src=".$val['pic']." width='80%'><a pos=".$val['position']." href=".'javascript:void(0)'.">点击修改</a><a href=".'javascript:void(0)'." class='dss' pos=".$val['position']." dss=".$val['id']." style='margin-left:250px'>删除</a></td>";
            }
        }
        ?>
    </tr>
...
<script type="text/javascript">
    ...
    $('.dss').click(function(){
        ...
        //alert(fid);
        //alert(111);
        $.getJSON('<?php echo $this->get_url('nation','dels')?>',{id:dss,val:v,fid:fid,gid:gid,epg:epg},function(d){
            if(d.code == 200){
                alert("删除成功");
                location.reload();
            }else{

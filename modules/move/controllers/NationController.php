...
class NationController extends MController{
	...
	$nid=$_GET['nid'];
	   ...
	   $epg=28;
        ...
        $mvcoui = MvCouiManager::getAll($gid,$epg,99);
        $mvui = MvUiManager::getAll($gid,$epg,99);
        $xiaotu = MvUiManager::getAll($gid,$epg,1);
        $xiaotus = MvCouiManager::getAll($gid,$epg,1);
        $htmls = HTML::movebk($mvcoui,$xiaotus);
        //print_r($xiaotu);die();
        $html = HTML::move($mvui,$xiaotu);
        if(!empty(MvCoui::model()->findAll("flag=1 and delFlag=1 and gid=$gid")) || !empty(MvCoui::model()->findAll("flag=1 and delFlag=5 and gid=$gid"))){
            $flag=1;
        }else if(!empty(MvCoui::model()->findAll("flag=1 and delFlag=2 and gid=$gid")) || !empty(MvCoui::model()->findAll("flag=1 and delFlag=6 and gid=$gid"))){
            $flag=2;
        }else{
            $flag=0;
        }
        $message = MvCoui::model()->findAll("flag=1 and delFlag=4 and gid=$gid ");
        $this->render("index",array('html'=>$html,'mvui'=>$mvui,'xiaotu'=>$xiaotu,'gid'=>$gid,'epg'=>$epg,'htmls'=>$htmls,'mvcoui'=>$mvcoui,'xiaotus'=>$xiaotus,'flag'=>$flag,'message'=>$message));
    }
    ...
    public function actionUpload(){
        ...
        $mvui = MvCouiManager::getAll($_GET['gid'],$_GET['epg'],99);
        $xiaotu = MvCouiManager::getAll($_GET['gid'],$_GET['epg'],1);
...
//                'w'=>$w
    ...
    public function actionAdd(){
        ...
        //if(empty($_POST['cp'])) $this->die_json(array('code'=>404,'msg'=>'牌照方不能为空'));
        //echo $_POST['key'];
        $mvui = MvCoui::model()->findByAttributes(array('position'=>$_POST['position'],'type'=>$_POST['type'],'id'=>$_POST['id']));
        //print_r($ui['id']);
        if(!$mvui){
            $mvui = new MvCoui();
            $mvui->addTime = time();
        ...
        //初始化
        /*$url = "http://172.17.91.196:8080/curl.php";
        //pclose(popen('php $url &', 'r'));
        //$url = "http://117.144.248.58:8082/hello.php";
        $post_data = array ("img" => $img);
        $ch = curl_init();
    ...
    public function actionSelv(){
        ...
        $res = MvUi::model()->find("gid=$gid and title='$title' and position='$pos'");
        $cid=$res->attributes['id'];
        //$cid=$_REQUEST['cid'];
        $mvui = MvCoseuiManager::getAll($cid);
    ...
    public function actionAdds(){
        ...
        //if(empty($_POST['cp'])) $this->die_json(array('code'=>404,'msg'=>'牌照方不能为空'));
        //echo $_POST['key'];
        $mvui = MvCoseui::model()->findByAttributes(array('pos'=>$_POST['position'],'type'=>$_POST['type'],'id'=>$_POST['id']));
    ...
    public function actionDel(){
        ...
        $del = MvCoui::model()->updateAll(array('delFlag'=>5),'id=:id',array(':id'=>$id));
        if(!$del){
    ...
    public function actionSedel(){
        ...
        $del = MvCoseui::model()->updateAll(array('flag'=>5),'id=:id',array(':id'=>$_REQUEST['id']));
        if(!$del){
            $this->die_json(array('code'=>404,'msg'=>'删除失败'));
        }
    ...
    public function actionTodel(){
        ...
        $pos = $_REQUEST['pos'];
        $gid = $_REQUEST['gid'];
        //$result=Yii::app()->db->createCommand()->delete('{{mv_coui}}', "gid='$gid' and epg='$epg' and position='$pos'");
        $del = MvCoui::model()->updateAll(array('delFlag'=>5),'id=:id',array(':id'=>$id));
        if(!$del){
            $this->die_json(array('code'=>404,'msg'=>'删除失败'));
        }
    ...
    public function actionRelease(){
        ...
        $coui = MvCoui::model()->findAll("gid=$gid and delFlag in(3,6)");
        foreach($coui as $k=>$v){
            /*$ui = MvUi::model()->find("gid=$gid and position='{$v['position']}' and addTime={$v['addTime']} ");
            if(!$ui){
                $ui = new MvUi();
            }
            ...
            $ui->addTime =$v->attributes['addTime'];
    ...
    public function curlPosts($url, $data = null)
    {
       $ch = curl_init(); 
$curl_opt = array( 
  CURLOPT_URL, $url,
     ...
     public function RecordAdd($data,$pic,$flag)
    {
        ...
        if(!empty($data['tType'])){
            ...
            {   
                ...
                case 5: $tType='全屏大图';  break;
                case 6: $tType='二级专题页';break;
                case 7: $tType='咪咕音乐';  break;
                case 8: $tType='咪咕视讯'; break;
                case 9: $tType='咪咕学堂'; break;
                case 10: $tType='糖果乐园'; break;
                case 11: $tType='咪咕爱唱'; break;
                case 12: $tType='和动漫';   break;
            }
        }else{
            $tType = '';
        }

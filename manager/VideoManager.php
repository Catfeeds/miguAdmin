...
$sql_where .=" or (a.flag = 0";
	...
	if($_SESSION['auth']==1){
        	...
        	$sql_select = 'select a.*,b.name,b.id as gid,d.id as workid,c.type,c.uid';
	}

        $sql_from = " from yd_ver_upload as a left join yd_ver_station as b on a.stationId=b.id ";
	
	if($_SESSION['auth']!=1){
        	$sql_join = " left join yd_ver_work as d on b.id=d.stationId and d.flag=8 inner join yd_ver_review_work as c on c.workid=d.id and c.uid={$_SESSION['userid']}";
	}else{

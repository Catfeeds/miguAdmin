...
/**
 * This is the model class for table "{{mv_content}}".
 *
 * The followings are the available columns in table '{{mv_content}}':
 * @property integer $id
 ...
 */
class MvContent extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('addTime, upTime, width, height, delFlag, order', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>100),
			array('type, cp', 'length', 'max'=>2),
			array('tType, epg', 'length', 'max'=>1),
			...
			array('id, title, type, tType, param, action, pic, cp, epg, addTime, upTime, position, gid, width, height, x, y, delFlag, order', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return MvContent the static model class
	 */
